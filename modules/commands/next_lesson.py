...
def get_next_lesson(message: Message) -> str or Embed:
    ...
    def process(time: datetime) -> tuple[str, str]:
        ...
        if next_lesson[0]:
            if lesson["period"] >= 20:
                # Currently lesson
                lesson_end_datetime = util.get_time(lesson["period"] % 20, time, True)
        ...
        return (
            f"{Emoji.INFO} Następna lekcja {group}to **{temp_str}**{when} o godzinie "
            f"__{next_period_time}__{countdown}."
        ), util.get_lesson_link(lesson["name"])
